...
// `input` may be a parsed RWS universe object, or a raw string — auto-detected:
// a leading `<` routes to the Semantic-Layer export ingest (ingestBobjSdkXml,
// which carries the physical columns + object SELECTs the RWS REST endpoint does
// NOT expose); anything else is parsed as RWS JSON. Both normalize to the same
// IR, so convertBobjIR stays the single shared core.
export function convertBobjToSigma(input, options = {}) {
    return convertBobjIR(parseBobjInput(input), { ...options, inputKind: detectBobjInputKind(input) });
}
/** Classify the raw input so the conversion can tell the user, up front, whether
 *  the file they provided carries a join graph:
...
export function detectBobjInputKind(input) {
...
export function convertBobjIR(uni, options = {}) {
    resetIds();
    const { connectionId = '<CONNECTION_ID>', database: dbOverride = '', schema: schOverride = '', modelName, tableMap, columnMap, inputKind } = options;
    const warnings = [];
    // Tell the user, up front, whether the file they provided can carry joins —
    // the #1 cause of a low-column workbook is an outline-only input silently
    // producing a joinless model.
    ...
    for (const join of uni.joins) {
        ...
        if (dir === 'left-many')
            ...
            if (lHasM !== rHasM)
                leftIsSource = lHasM; // the measure-bearing (fact) side is the many side
            else {
                leftIsSource = true;
    ...
    // Guard: a multi-table universe that produced NO relationships yields a
    // low-fidelity model — the denormalized View can only traverse to dimension
    // columns via a relationship, so a workbook bound to it sees just the one
    // fact table's columns. This used to happen silently; surface it loudly.
...
// OUTLINE — object names, datatypes, folders — NOT the relational bindings (each
// object's SELECT/WHERE) nor the data foundation (physical tables, columns,
// joins). To migrate the actual warehouse columns and calculations you need a
// Semantic Layer SDK / Information Design Tool export of the data foundation +
// business layer (see scripts/extract-universe-sdk.groovy). This ingest parses
// that XML into the SAME IR normalizeBobjUniverse produces, so convertBobjIR is
// unchanged. It is deliberately SHAPE-TOLERANT (SAP publishes no .dfx/.blx
// schema): tables/joins from a <dataFoundation> subtree if present (else whole
// doc), objects + filters from a <businessLayer> subtree (else whole doc);
// tag/attribute names matched case-insensitively across the common variants.
export function ingestBobjSdkXml(xml) {
    const doc = parseXml(xml);
    const uni = findFirst(doc, ['universe', 'unx', 'businessuniverse']) || doc;
    const name = uni.attrs.name || textOf(findFirst(uni, ['name', 'universename'])) || 'BusinessObjects Universe';
    ...
    for (const j of findAll(dfScope, n => n.tag === 'join')) {
        const expression = textOf(findFirst(j, ['expression', 'statement', 'sql', 'definition'])) ||
    ...
    for (const f of findAll(blScope, n => /^(filter|condition|predefinedfilter)$/.test(n.tag))) {
        const where = textOf(findFirst(f, ['where', 'whereclause', 'expression', 'sql', 'definition'])) ||
            f.attrs.where || undefined;
        if (!where && !f.attrs.name)
            continue;
        filters.push({ name: f.attrs.name || textOf(findFirst(f, ['name'])) || 'Filter', where });
    }
    // Backfill any table referenced by an object SELECT but absent from the data foundation.
    for (const o of objects)
        for (const { table } of parseTableColTokens(o.select)) {
            const key = tableKeyOf(table);
            if (!tableMap.has(key))
...
function walkBlNodes(nodes, out, parentClass) {
    for (const node of nodes) {
        ...
        const selectNode = (binding && findFirst(binding, ['select', 'selectclause', 'expression'])) ||
        ...
        if (select || (tagIsObj && node.attrs.name)) {
            ...
            if (!q && /^(dimension|measure|attribute|detail)$/.test(node.tag))
                q = node.tag;
            let kind = 'dimension';
            if (/measure/.test(q))
                kind = 'measure';
            else if (/detail|attribute/.test(q))
                kind = 'detail';
            out.push({
                ...
                select,
                aggregation: node.attrs.aggregation || node.attrs.aggregationfunction ||
                    node.attrs.projectionfunction || node.attrs.function || undefined,
                description: node.attrs.description || node.attrs.help ||
                    textOf(findFirst(node, ['description', 'help'])) || undefined,
            });
            continue; // don't descend into a leaf object
        }
        if (node.children.length)
            walkBlNodes(node.children, out, parentClass);
    }
}
...
// and are entity-decoded. Kept in lock-step with bobj.ts + the browser mirror.
function parseXml(xml) {
    ...
    while (i < n) {
        ...
        if (lt < 0) {
            addText(top(), s.slice(i));
            break;
        }
        if (lt > i)
            addText(top(), s.slice(i, lt));
        if (s.startsWith('<!--', lt)) {
...
function findFirst(node, tags) {
    for (const c of node.children)
        if (tags.includes(c.tag))
            return c;
    for (const c of node.children) {
        const r = findFirst(c, tags);
        if (r)
            return r;
    }
    return null;
...
function textOf(node) { return node ? (node.text || '').trim() : ''; }
 ...
 *  Matches the first equality of two qualified column refs, each of which may be
 *  2-or-more dotted, optionally-quoted segments — so schema/catalog-qualified
 *  3-part names ("DWH"."CUSTOMER"."ID" = "DWH"."SALES"."CUST_ID") resolve, not
 *  just bare Table.Col. The last two segments of each ref are taken as table.col. */
function parseJoinKeys(join) {
    const expr = join.expression || '';
    // One identifier segment: a quoted/bracketed token (spaces allowed inside) OR a
    // bare unquoted identifier (no spaces — so it can't swallow " AND "/operators in
    // a multi-condition join).
    const seg = '(?:["\'`\\[][^"\'`\\]]+["\'`\\]]|[A-Za-z_]\\w*)';
