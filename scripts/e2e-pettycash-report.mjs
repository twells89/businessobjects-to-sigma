#!/usr/bin/env node
/**
 * Pinned Crystal source+PDF oracle → Snowflake → Sigma report → PDF.
 */
...
const manifest = JSON.parse(
  readFileSync('fixtures/crystal/pettycash-monthly-report.source.json', 'utf8'),
);
const artifacts = resolve(
  process.env.PETTYCASH_ARTIFACTS_DIR || 'artifacts/crystal/pettycash-e2e',
);
mkdirSync(artifacts, { recursive: true });
const rptPath = resolve(artifacts, 'MonthlyReport.rpt');
const referencePdfPath = resolve(artifacts, 'Monthly_Report.crystal.pdf');
const irPath = resolve(artifacts, 'MonthlyReport.crystal-ir.json');
const specPath = resolve(artifacts, 'MonthlyReport.sigma-report.json');
const ledgerPath = resolve(artifacts, 'MonthlyReport.degradations.json');
...
async function main() {
  ...
  const sourceTable = manifest.sigma.sourceTable;
  ...
  run('python3', [
    'scripts/seed-pettycash-snowflake.py',
    '--database',
    database,
    '--schema',
    schema,
  ...
  console.log(`Synced Sigma source metadata: ${database}.${schema}.${sourceTable}`);

  const schemaVersion = await referenceReportSchemaVersion();
  const converted = convertPettyCashToReport(ir, {
    folderId,
    connectionId,
    database,
    schema,
    sourceTable,
    schemaVersion,
    reportName: `PettyCash Monthly Report — Crystal E2E ${new Date().toISOString().slice(0, 16)}`,
  });
  ...
  const detail = await queryReportElement(
    created.reportId,
    manifest.sigma.detailElementId,
  );
  const queryProblems = scanErrors(detail);
  if (queryProblems.length) {
    throw new Error(`Report element query problems: ${queryProblems.join('; ')}`);
  }
  writeFileSync(
    resolve(artifacts, `${created.reportId}.query.json`),
...
main().catch(error => {
  console.error('e2e-pettycash-report failed:', error.message);
  process.exit(1);
});
